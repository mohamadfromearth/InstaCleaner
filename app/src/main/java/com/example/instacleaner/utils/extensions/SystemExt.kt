...
fun getPackageInfo(): PackageInfo {
    return App.getInstance().packageManager.getPackageInfo(App.getInstance().packageName, 0)
}
...
fun getStatusBarHeight(): Int {
    var result = 0
    val resourceId = App.getInstance().resources.getIdentifier("status_bar_height", "dimen", "android")
    if (resourceId > 0) {
        result = App.getInstance().resources.getDimensionPixelSize(resourceId)
    }
    return result
}

fun getScreenDensity(): Float {
    return App.getInstance().resources.displayMetrics.density
}
...
fun copyToClipboard(text: String) {
    val clipboard = App.getInstance().getSystemService(Application.CLIPBOARD_SERVICE) as android.content.ClipboardManager
...
fun convertUriToPath(contentUri: Uri): String {
    ...
    try {
        ...
        cursor = App.getInstance().contentResolver.query(contentUri, proj, null, null, null)
...
fun isAppAvailable(appName: String): Boolean {
    return try {
        App.getInstance().packageManager.getPackageInfo(appName, 0)
        true
...
fun toast(message: Int) {
    Toast.makeText(App.getInstance(), App.getInstance().resources.getString(message), Toast.LENGTH_SHORT).show()
}
...
fun snack(view: View?, message: String, long: Boolean = false) {
    view?.let {
        ...
        //tv.typeface = App.getInstance().m.fontLight
        snack.show()
    }
}
...
fun isPhonePluggedIn(): Boolean {
    ...
    val batteryIntent: Intent? = App.getInstance().registerReceiver(null, IntentFilter(Intent.ACTION_BATTERY_CHANGED))
...
fun getBatteryPercentage(): Int {
    ...
    val batteryStatus = App.getInstance().registerReceiver(null, iFilter)
...
fun hasPermission(permission: String): Boolean {
    val res = App.getInstance().checkCallingOrSelfPermission(permission)
...
fun pasteClipboard(): String {
    val clipboard = App.getInstance().getSystemService(Context.CLIPBOARD_SERVICE) as android.content.ClipboardManager
    var pasteData = ""
